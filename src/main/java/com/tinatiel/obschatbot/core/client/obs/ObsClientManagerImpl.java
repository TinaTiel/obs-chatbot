...
//            actionRequest.cancel(true);
  ...
  public void startClient() {
    if (obsRemoteController == null) {
      ...
      obsRemoteController.registerCloseCallback((statusCode, reason) -> {
        log.info(String
          .format("OBS Client closed with statusCode %s and reason '%s", statusCode, reason));
      ...
      try {
        ...
        throw new ClientException(
          "OBS Client failed to start within timeout (" + settings.getConnectionTimeoutMs()
            + "ms)", e);
      }
  ...
  public void stopClient() {
    ...
    if (obsRemoteController == null) {
      throw new ClientException("No client was available to stop");
    }
    ...
    try {
      ...
      throw new ClientException(
        "OBS Client failed to stop within timeout (" + settings.getConnectionTimeoutMs() + "ms)",
        e);
    } finally {
