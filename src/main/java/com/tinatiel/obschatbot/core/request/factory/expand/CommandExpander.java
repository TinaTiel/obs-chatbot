...
public interface CommandExpander {
  ...
  /**
   * Takes a command and expands it into a list of actions. The command should be checked for
   * cyclical actions before this is called, however in the worst case a wrapped StackOverFlow
   * exception will be thrown -- for example, caused by a TODO: Scripted Action
   *
   ...
   * @throws CyclicalActionsException if the actions cause a cyclical reference, may wrap a
   *                                  StackOverflowException.
   */
  List<Action> expand(Command command) throws CyclicalActionsException;

  /**
   * Statically checks for cyclical actions; e.g. if a Command contains Actions that point to a
   * Command upstream in the chain. Note, this does not catch any cycles caused by TODO: scripted
   * Actions.
   *
